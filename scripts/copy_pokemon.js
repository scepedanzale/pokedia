...
const fetchAllPokemon = () => {
    return axios(BASE_URL + '/pokemon?limit=10000')
        .then(response => {
...
const fetchDetail = async (pokemon) => {
    try {
        ...
        const specieRes = await axios(poke.species.url);
        ...
        if (err.response?.status === 404) {
            console.warn(`⚠️ Pokémon non trovato (${pokemon.name || pokemon.id})`);
            return null;
        } else {
            console.error(`❌ Errore per ${pokemon.name || pokemon.id}:`, err.message);
            return null;
        }
    }
}
...
const main = async () => {
    ...
    console.log(`Totale Pokémon trovati: ${allPokemon.length}`);
